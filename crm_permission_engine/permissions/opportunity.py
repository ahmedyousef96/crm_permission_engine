...
def has_permission(doc, user):
    """
    Visibility rules for a single Opportunity document.
    """
    ...
    if doc.opportunity_owner:
        return is_sales_person_in_user_tree(user, doc.opportunity_owner)
...
def permission_query(user, doctype=None):
    """
    SQL permission query for Opportunity list views.
    """
    ...
    # Assigned opportunities ? Sales Person Tree
    conditions.append(
        """
        `tabOpportunity`.`opportunity_owner` IN (
            SELECT e2.user_id
            FROM `tabEmployee` e1
    ...
    # Unassigned opportunities ? Territory Tree
    from crm_permission_engine.permissions.utils import (
    ...
    if ranges:
        ...
        conditions.append(
            """
            (
                (`tabOpportunity`.`opportunity_owner` IS NULL
                 OR `tabOpportunity`.`opportunity_owner` = '')
                AND `tabOpportunity`.`territory` IN (
